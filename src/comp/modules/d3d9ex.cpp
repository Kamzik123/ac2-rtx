...
#pragma region D3D9Device
	...
	HRESULT d3d9ex::D3D9Device::CreateTexture(UINT Width, UINT Height, UINT Levels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, IDirect3DTexture9** ppTexture, HANDLE* pSharedHandle)
	{
		...
		// Instrumentation: AC2's streaming reuses texture slots, which breaks
		// Remix's assumption that a texture is stable once created.
		if (SUCCEEDED(hr) && ppTexture && *ppTexture) {
			ac2_dump::on_create_texture(Width, Height, Levels, Usage, Format, Pool, *ppTexture);
		}
		return hr;
	}
	...
	{
		// Shadow every constant write. We cannot trust GetVertexShaderConstantF
		// under Remix: the game talks to a 32-bit bridge that IPCs to the 64-bit
		// host, and Get* state readback there is unreliable. Reading our own copy
		// is authoritative. (This is c0..c3 = g_WorldViewProj, c8..c11 = g_World,
		// c120..c245 = g_BoneMatrixArray.)
		ac2_ff::on_set_vs_constant_f(StartRegister, pConstantData, Vector4fCount);
	...
	{
		const auto hr = m_pIDirect3DDevice9->CreatePixelShader(pFunction, ppShader);
		// After the real create: work out which sampler slot is the diffuse.
		if (SUCCEEDED(hr) && ppShader && *ppShader) {
			ac2_dump::on_create_pixel_shader(pFunction, *ppShader);
		}
		return hr;
	}
